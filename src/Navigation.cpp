...
Navigation::Navigation()
    : Node("navigation"),
      node_odom_nav(rclcpp::Node::make_shared("odom_node")),
      req_pos_y(0.0) {
  //   node_odom_nav = rclcpp::Node::make_shared("odom_node");
...
bool Navigation::search_obj() {
  ...
  auto odom_sub = node_odom_nav->create_subscription<ODOM>(
      "odom", 10, std::bind(&Navigation::search, this, _1));
...
bool Navigation::move_to_disposal_zone() {
  RCLCPP_INFO(this->get_logger(), "Pickedup waste, Moving to track bin");
  check_odom = false;
  auto odom_sub = node_odom_nav->create_subscription<ODOM>(
      "odom", 10, std::bind(&Navigation::disposal, this, _1));
  POSE rpyGoal;
  rpyGoal.header.frame_id = "map";
  rpyGoal.header.stamp = this->get_clock()->now();
